...
// Помощник JamesHelper умеет ходить по протоколу telnet на почтовый сервер и создавать там польз-ей и получать почту по протоколу pop3. Он создает
// поль-ля так: для этого исп-ся TelnetClient, к-ый явл-ся частью библиотеки commons.net и при инициализации JamesHelper созд-ся телнет клиент. В самом
// начале создается почтовая сессия mailSession.
